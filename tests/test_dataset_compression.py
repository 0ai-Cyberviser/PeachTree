...
def test_compression_stats_creation():
    ...
    assert stats.space_saved_bytes == 2500
...
def test_compression_stats_to_dict():
    ...
    assert data["space_saved_bytes"] == 2500
...
def test_dataset_compressor_compression_levels(tmp_path: Path):
    ...
    # Best compression should result in smaller file
...
def test_compression_analyzer_analyze_compression_ratio(tmp_path: Path):
    ...
    ratio = analyzer.analyze_compression_ratio(dataset, CompressionAlgorithm.GZIP)
...
def test_batch_compressor_compress_datasets(tmp_path: Path):
    ...
    assert (output_dir / "input1.jsonl.gz").exists()
    assert (output_dir / "input2.jsonl.gz").exists()


def test_batch_compressor_decompress_datasets(tmp_path: Path):
    """Test batch decompression of multiple datasets."""
    compressor = BatchCompressor(CompressionAlgorithm.GZIP)
    ...
    # Compress first
    compressor.compress_datasets([input1, input2], compressed_dir)
    
    # Decompress
    compressed1 = compressed_dir / "input1.jsonl.gz"
    compressed2 = compressed_dir / "input2.jsonl.gz"
    
    sizes = compressor.decompress_datasets([compressed1, compressed2], decompressed_dir)
    
    assert len(sizes) == 2
    assert all(s > 0 for s in sizes)
    assert (decompressed_dir / "input1.jsonl").exists()
    assert (decompressed_dir / "input2.jsonl").exists()
...
def test_batch_compressor_get_statistics():
    ...
    assert stats.average_compression_ratio == 0.5
    assert stats.space_saved_bytes == 1500
...
def test_streaming_compressor_chunk_size(tmp_path: Path):
    ...
    compressor = StreamingCompressor(CompressionAlgorithm.GZIP, chunk_size=4096)
...
def test_compression_analyzer_best_vs_fast(tmp_path: Path):
    """Test that best compression is better than fast."""
    dataset = tmp_path / "dataset.jsonl"
    test_data = '{"id": 1, "content": "test data repeated"}\n' * 100
    dataset.write_text(test_data)
    
    fast_compressor = DatasetCompressor(CompressionAlgorithm.GZIP)
    best_compressor = DatasetCompressor(CompressionAlgorithm.GZIP)
    
    fast_file = tmp_path / "fast.gz"
    best_file = tmp_path / "best.gz"
    
    fast_meta = fast_compressor.compress_file(dataset, fast_file, level=1)
    best_meta = best_compressor.compress_file(dataset, best_file, level=9)
    
    # Best compression should achieve better ratio
    assert best_meta.compression_ratio <= fast_meta.compression_ratio
